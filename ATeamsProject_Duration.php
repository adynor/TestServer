...
?>
  ...
  <div ng-show="successMessagebool " id="success-alert" class="alert alert-success"> {{successMessage}}</div>
  <tr style="width:100%; border: 1px solid darkorange;" ng-repeat="value in data | filter:searchFilter"> 
      ...
      <td>
          <input id="showB{{value.TM_id}}" ng-model="value.TM_PR_Duration" type="text"/><button style="    float: right;" type="button" class="btn btn-info btn-xs" ng-click="changeduration(value.TM_id,value.PR_id,value.TM_PR_Duration);">Change</button>
      </td>
...
$scope.url5="changeteamduration.php";
      ...
      $scope.changeduration = function(t,p,id) {

        $scope.prid=p;
        $scope.tmid=t;
         $scope.prduration=id;
            $http.post($scope.url5,{"tmid":$scope.tmid,"prid":$scope.prid,"prduration":$scope.prduration}).
                        success(function(data,status) {
                    
                     if(status==200){
                         $scope.successMessage = "Project Duration Updated successfully";
